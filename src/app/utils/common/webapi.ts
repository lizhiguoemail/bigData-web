...
import { HttpContentType, HttpHelper, HttpMethod, HttpRequest } from '../angular/http-helper';
import { FailResult, Result, StateCode } from '../core/result';
...
export class WebApi {
     ...
     * @param body Http主体
     */
    static patch<T>(url: string, body?): WebApiRequest<T> {
        return new WebApiRequest<T>(HttpHelper.patch<Result<T>>(url, body));
    }
...
export class WebApiRequest<T> {
    ...
    handlePromise(): Promise<any> {
        return this.request.handlePromise();
    }
