...
export let toNumber = (value, precision?, isTruncate?: boolean) => {
    ...
    if (isTruncate) {
        return parseFloat(value.substring(0, value.indexOf('.') + parseInt(precision, 10) + 1));
    }
 ...
 * @param year 增加的年
 */
export function nowDateAddYear(year: number) {
 ...
 * @param input 加密字符
 */
export function crypto(input: string): string {
    const u8: any = Md5.hashStr(input, true);
    const base64Str = btoa(String.fromCharCode.apply(null, new Uint8Array(u8.buffer)));
    return base64Str;
}
