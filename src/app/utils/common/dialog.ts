...
export class Dialog {
    ...
    static open(options?: IDialogOptions) {
        ...
        const dialog: NzModalService = Dialog.getModalService();
        const dialogRef: NzModalRef = dialog.create(this.toOptions(options));
     ...
     * 转换配置
     */
    private static toOptions(options: IDialogOptions): ModalOptions {
        return {
            ...
            nzOnCancel: data => {
                if (data.tag === true) {
                    if (options.onBeforeClose) {
                        options.onBeforeClose(data.result);
                    }
                    return;
                }
                if (options.onBeforeClose) {
     ...
     */
    static closeAll() {
        const dialog = Dialog.getModalService();
        dialog.closeAll();
    }
