...
export class CheckboxGroup implements OnInit, AfterViewInit, OnDestroy {
    /**
    ...
    onModelChange(data: CheckboxOption[]) {
        ...
        const values = data.filter(t => t.checked).map(t => t.value);
...
export function requiredValidator(data: CheckboxOption[]): ValidatorFn {
    return (control: AbstractControl): { [key: string]: any } | null => {
        const isValid = data.some(t => t.checked);
