import { AfterContentInit, Component, EventEmitter, forwardRef, Input, Output, ViewChild } from '@angular/core';
...
export class Tree implements AfterContentInit {
    ...
    query( option?: {
        ...
        option = option || {};
        const url = option.url || this.url;
        if ( !url ) {
            return;
        }
        const param = option.param || this.queryParam;
