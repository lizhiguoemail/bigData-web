...
export abstract class TableBase<T extends IKey> implements OnInit {
    /**
     * 表格密码
     */
    tableSize: string;
    ...
    @Input() url: string;
    /**
     * 删除地址，注意：采用delete提交，范例：/api/test/{id}
     */
    @Input() deleteUrl: string;
    /**
     * 批量删除地址，注意：由于支持批量删除，所以采用Post提交，范例：/api/test/delete
     */
    @Input() deleteBatchUrl: string;
    /**
     * 查询参数
     */
    ...
    query(options?: {
        ...
        options = options || {};
        const url = options.url || this.url || (this.baseUrl && `/api/${this.baseUrl}`);
        if (!url) {
            return;
        }
    ...
    delete(options?: {
        ...
        this.deleteRequest(options.button, id, options.handler, options.url);
    }

    /**
     * 发送删除请求
     */
    private deleteRequest(button?, id?: string, handler?: () => void, url?: string) {
        url = url || (this.deleteUrl && `/api/${this.deleteUrl}/${id}`) || (this.baseUrl && `/api/${this.baseUrl}/${id}`);
        if (!url) {
            console.log('表格deleteUrl未设置');
            return;
        }
        util.webapi.delete(url).button(button).handle({
            ok: () => {
    ...
    deleteBatch(options?: {
        ...
        options = options || {};
        const ids = options.ids || this.getCheckedIds();
        if (!ids) {
            util.message.warn(config.deleteNotSelected);
            return;
        }
        util.message.confirm(config.deleteConfirm, () => {
            this.deleteBatchRequest(options.button, ids, options.handler, options.url);
        });
    }
     ...
     * 发送删除请求
     */
    private deleteBatchRequest(button?, ids?: string, handler?: () => void, url?: string) {
        url = url || (this.deleteBatchUrl && `/api/${this.deleteBatchUrl}/delete`) || (this.baseUrl && `/api/${this.baseUrl}/delete`);
        if (!url) {
            console.log('表格deleteBatchUrl未设置');
            return;
        }
    ...
    checkIds(ids) {
        ...
        if (!ids.some) {
            const item = this.dataSource.find(data => data.id === ids);
            this.checkedSelection.select(item);
            return;
        }
        const list = this.dataSource.filter(data => ids.indexOf(data.id) > -1);
        list.forEach(item => {
    ...
    private initLineNumbers(data) {
        const result = new PagerList<T>(data, this.queryParam.page, this.queryParam.pageSize);
        result.initLineNumbers();
    }
    ...
    removeRows(ids?: string[]) {
        ...
        const result = util.helper.remove(this.dataSource, row => ids.findIndex(id => row.id === id) > -1);
