 ...
 */
@Component({
  selector: 'app-shanxi-map',
  template: `
      <div class="shanxi-map-container">
          <div *ngIf="!loading" class="chart" id="shanxi-map-container"></div>
          <app-loader-chart *ngIf="loading"></app-loader-chart>
      </div>
    `,
  styles: [
    `
      .shanxi-map-container {
        height: 100%;
        margin: 0 auto;
      }
      .shanxi-map-container .chart {
          height: 100%;
          width: 100%;
          margin: 0 auto;
      }
...
export class ShanxiMapComponent extends ChartWrapperBase implements OnInit, AfterContentInit {
  /**
   * 山西省代码
   */
  shanxiCode = '140000';
  ...
  render(areaCode) {
    ...
    this.$get(`/assets/json/geo/${areaCode}.json`).then((mapData) => {
      const chart = new Chart({
        container: 'shanxi-map-container',
        autoFit: true,
        padding: areaCode === this.shanxiCode ? [30, 90, 30, 90] : [30, 50, 30, 50], // 上、右、下、左
      });
      chart.on('plot:click', (ev) => {
        let code: string = this.shanxiCode;
        let parent: string = this.shanxiCode;
        if(ev.data){
          if(!ev.data.data) return;
          if(ev.data.data.parent.substring(4,6)!='00') return;
          const data = ev.data.data;
          code = data.code;
          parent = data.parent;
          this.prevData = data;
        } else {
          if(areaCode == this.shanxiCode) return;
          if(this.prevData.parent == areaCode) {
            code = this.shanxiCode;
          } else {
            code = this.prevData.parent;
          }
        }
        let tmpcode = '';
        if(code == this.shanxiCode) {
          tmpcode = '14';
        } else if(code.substring(4,6)=='00') {
          tmpcode = code.substring(0,4);
        } else {
          tmpcode = code;
        }
        util.webapi.get<any>(`api/ldl_v/data/map?code=${tmpcode == '14' ? '' : tmpcode}`).handle({
          ok: (resp) => {
            this.loadData(resp);
