 ...
 */
@Component({
  selector: 'app-shanxi-skills-map',
  template: `
      <div class="shanxi-skills-map-container">
        <div *ngIf="!loading" class="chart" id="shanxi-skills-map-container"></div>
        <app-loader-chart *ngIf="loading"></app-loader-chart>
      </div>
    `,
  styles: [
    `
    .shanxi-skills-map-container {
      height: 100%;
      margin: 0 auto;
    }
    .shanxi-skills-map-container .chart {
        height: 100%;
        width: 100%;
        margin: 0 auto;
    }
...
export class ShanxiSkillsMapComponent extends ChartWrapperBase implements OnInit {
  /**
   * 山西省代码
   */
  shanxiCode = '140000';
  ...
  render(areaCode) {
    ...
    this.$get(`/assets/json/geo/${areaCode}.json`).then((mapData) => {
      const chart = new Chart({
        container: 'shanxi-skills-map-container',
        autoFit: true,
        height: 900,
