...
import { SelectItem, SelectList, SelectOption, SelectOptionGroup } from '@utils';
import { util } from '@utils';
import { FormControlWrapperBase } from '@utils';

/**
 * 工种下拉列表包装器
 */
@Component({
  selector: 'app-common-trades',
  template: `
...
export class CommonTradesComponent extends FormControlWrapperBase implements OnInit {
  /**
   * 按组显示
   */
  isGroup: boolean;
  ...
  ngOnInit() {
    this.data = [
      ...
      { text: '家政服务员', value: '家政服务员' },
      { text: '保健按摩师（准）', value: '保健按摩师（准）' },
      { text: '中式面点师', value: '中式面点师' },
      { text: '化妆师', value: '化妆师' },
      { text: '育婴员', value: '育婴员' },
      { text: '保育员', value: '保育员' },
      { text: '计算机操作员', value: '计算机操作员' },
      { text: '美容师（准）', value: '美容师（准）' },
